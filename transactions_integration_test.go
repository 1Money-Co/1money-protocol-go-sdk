...
func TestGetTransactionByHash(t *testing.T) {
	...
	client := NewClientWithCustomUrl("http://127.0.0.1:18555", WithTimeout(10*time.Second))
	// for create/mint related transaction, can check cp=1 related transactions to get the hash to test
	hash := "0x40dfd368031eebb8a619bd03bfa91abb7f7bc2e7bc5961b8bab304f59dfca3cc"
	...
	case "TokenCreate":
		if tokenData, ok := result.Data.(*TokenCreateData); ok {
			fmt.Printf("Token Symbol: %s\n", tokenData.Symbol)
		}
	case "TokenTransfer":
		if transferData, ok := result.Data.(*TokenTransferData); ok {
			fmt.Printf("Transfer Amount: %s\n", transferData.Value)
		}
	case "TokenMint":
		if mintData, ok := result.Data.(*TokenMintData); ok {
...
func TestGetEstimateFee(t *testing.T) {
	client := NewTestClient()
	from := common.HexToAddress("0xfcecaf244ce223050980038c4fe2328e7580afd9")
	token := common.HexToAddress("0x354312ce56a578c98559154Dd7A50F5C08D17270")
...
func TestSendPayment(t *testing.T) {
	client := NewTestClient()
	accountNonce, err := client.GetAccountNonce(context.Background(), TestOperatorAddress)
	if err != nil {
	...
	payload := PaymentPayload{
		...
		Token:            common.HexToAddress(TestTokenAddress),
	}
	// Sign the payload
	signature, err := client.SignMessage(payload, TestOperatorPrivateKey)
	if err != nil {
